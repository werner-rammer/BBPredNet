...
# Each record in the test/validation data contains:
    ...
    * observed damage for the focal pixel yes/no (note that the focal pixel in the above image is of course cleared) [362]
...
bbtrain = np.loadtxt("bbtrainall.txt", dtype='int8')
...
model.fit({'input': X, 'in_climate': Xclim}, {'target': Y}, n_epoch=50, batch_size=512,
           validation_set=({'input': testX,'in_climate': testXclim}, {'target': testY}),
           snapshot_step=None, show_metric=True, run_id='BBPredNet E2')
...
model.load("e:/tmp/snapshots/9681")          
...
""" Helper function: 
## calculate precision/recall for a dataset.
"""
...
####################################################
 ...
 of the test set. The optimal threshold is for this 
 case, when the observed damage is equal the predicted 
 damage (also maximal F1 score).
####################################################
"""
...
# the best split is found at 0.845 
...
validationScores(0.845, mpredict, testY)


# save for further analysis, creation of maps
np.savetxt('predict_bb_exp2.txt', mpredict)
